...
(s/def ::catalog-tries
  (s/map-of ::shard ::shard-values))

#_
(s/assert ::catalog-tries
          '{ ;; L0 files
            [0 nil []] {:max-block-idx 1 :live () :nascent () :garbage ({:block-idx 42, :recency nil :level 0, :part [], :state :garbage}
                                                                        {:block-idx 44, :recency nil :level 0, :part [], :state :garbage})}
