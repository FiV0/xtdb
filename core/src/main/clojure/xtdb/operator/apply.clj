    ...
    (let [[col-name _expr] (first (:mark-join mark-spec))]
      (reify ModeStrategy
        (accept [_ dep-cursor dep-out-writer idxs in-idx]
          (let [out-writer (.colWriter dep-out-writer (types/col-type->field (name col-name) [:union #{:null :bool}]))]
            (.add idxs in-idx)
      ...
      (reify ModeStrategy
        (accept [_ dep-cursor dep-out-writer idxs in-idx]
          (doseq [[col-name col-type] dependent-col-types]
            (.colWriter dep-out-writer (types/col-type->field (name col-name) col-type)))
        ...
        (accept [_ dep-cursor dep-out-writer idxs in-idx]
          ...
          (let [match? (boolean-array [false])]
            ...
            (when-not (aget match? 0)
              (.add idxs in-idx)
              (doseq [[col-name col-type] dependent-col-types]
                (vw/append-vec (.colWriter dep-out-writer (types/col-type->field (name col-name) col-type))
