...
(defn max-direct-memory
  ...
  []
  (if-let [size-string (get (System/getProperties) "MaxDirectMemory")]
    (let [^long size (parse-long (subs size-string 0 (dec (count size-string))))]
      (case (last (str/lower-case size-string))
