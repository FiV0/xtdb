    ...
    fun buildFromRelation(inRel: RelationReader): RelationMapBuilder {
        ...
        return object : RelationMapBuilder {
            override fun add(idx: Int) {
                add(computeHashBitmap(hasher.hashCode(idx)), idx);
            }
            
            override fun addIfNotPresent(idx: Int): Int {
                val hashBitmap = computeHashBitmap(hasher.hashCode(idx))
                val outIdx = findInHashBitmap(hashBitmap, comparatorLazy, idx, false)
                return if (outIdx >= 0) {
                    outIdx
                } else {
                    add(hashBitmap, idx)
    ...
    fun probeFromRelation(probeRel: RelationReader): RelationMapProber {
        ...
        return object : RelationMapProber {
            override fun indexOf(idx: Int, removeOnMatch: Boolean): Int{
                ...
                return findInHashBitmap(hashBitmap, comparator, idx, removeOnMatch)
            }
            ...
            override fun forEachMatch(idx: Int, consumer: IntConsumer) {
                ...
                hashBitmap?.let { bitmap ->
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (comparator.applyAsInt(idx.toInt(), outIdx) == 1) {
            ...
            override fun matches(probeIdx: Int): Int {
                ...
                for (buildIdx in 0 until buildRowCount.toInt()) {
                    val res = comparator.applyAsInt(probeIdx.toInt(), buildIdx)
                    if (res == 1) {
                        return 1
                    }
                    acc = maxOf(acc, res)
