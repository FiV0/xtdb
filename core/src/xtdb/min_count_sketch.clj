...
(def ^:private offset (+ Short/BYTES Byte/BYTES))
(defn size [w d] (+ offset (* w d Long/BYTES)))

(defn create [{:keys [w d] :or {w 15 d 3}}]
  (let [s (size w d)]
    (doto ^MutableDirectBuffer (mem/allocate-buffer s)
      (.putShort 0 (short w))
