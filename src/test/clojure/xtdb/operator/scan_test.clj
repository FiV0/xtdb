...
(t/deftest test-trie-skew
  (let [uuid1 #uuid "00000000-0000-0000-0000-000000000000"
        uuid2 #uuid "10000000-0000-0000-0000-000000000000"]
    (with-open [node (node/start-node {:xtdb/live-chunk {:rows-per-block 16 :rows-per-chunk 16}})]
      (->> (repeat 14 [:put :xt_docs {:xt/id uuid1}])
           vec
           (xt/submit-tx node))
      (xt/submit-tx node [[:put :xt_docs {:xt/id uuid2}]])

      (->> (repeat 14 [:put :xt_docs {:xt/id uuid2}])
           vec
           (xt/submit-tx node))
      (xt/submit-tx node [[:put :xt_docs {:xt/id uuid1}]])
...
(t/deftest test-chunk-boundary
  (with-open [node (node/start-node {:xtdb/live-chunk {:rows-per-block 20, :rows-per-chunk 20}})]
    ...
    ;; FIXME to get a new live index
    (xt/submit-tx node [[:put :xt_docs {:xt/id 100}]])

    (t/is (= (set (for [i (range 101)] {:xt/id i}))
