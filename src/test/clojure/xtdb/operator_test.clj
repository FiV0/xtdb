...
(t/deftest test-absents-with-left-outer-join
  (t/is (= [{:a 12, :c 42, :b 12} {:a 0, :c nil, :b nil}]
           (tu/query-ra [:left-outer-join '[{a b}]
                         [::tu/blocks [[{:a 12}, {:a 0}]]]
                         [::tu/blocks [[{:b 12, :c 42}]]]])))

  #_(t/is (= [{:a 0, :c nil, :b nil}]
             (tu/query-ra [:left-outer-join '[{a b}]
                           [::tu/blocks [[{:a 0}]]]
                           [::tu/blocks [[{:b 12, :c 42}]]]])))

  (t/is (= [{:a 0, :c nil, :b nil}]
           (tu/query-ra [:left-outer-join '[{a b}]
                         [::tu/blocks [[{:a 0}]]]
                         [::tu/blocks [[{:b 12, :c {:foo 42}}]]]])))

  #_(t/is (= nil
             (tu/query-ra '[:project [b c]
                            [::tu/blocks [[{:b 12, :c {:foo 42}}]]]])))
  )
           ...
           (-> (tu/query-ra [:left-outer-join '[{a b}]
                             [::tu/blocks
                              [[{:a 12}, {:a 0}]
                               #_[{:a 12}, {:a 100}]]]
                             [::tu/blocks
                              [[{:b 12, :c {:foo 1}}, #_{:b 2, :c {:foo 1}}]
                               #_[{:b 12, :c {:foo 2}}, {:b 100, :c {:foo 2}}]]]]
                            {:preserve-blocks? true, :with-col-types? true})
               (update :res (partial mapv frequencies))))
        "testing left-outer-join with structs")
  #_(t/is (= {:res [{{:a 12, :c [1], :b 12} 1, {:a 12, :c [2], :b 12} 1, {:a 0, :c nil, :b nil} 1}
