...
(defn ->conn
  ...
  (^java.lang.AutoCloseable [frontend startup-params]
   (->conn frontend startup-params [{:user "all", :method :trust :address "127.0.0.1"}]))
  (^java.lang.AutoCloseable [frontend startup-params authn-records]
...
(deftest test-startup
  (let [{:keys [!in-msgs] :as frontend} (->recording-frontend [{:msg-name :msg-password :password "xtdb"}])]
    (with-open [_ (->conn frontend {"user" "xtdb" "database" "xtdb"} [{:user "xtdb", :method :password :address "127.0.0.1"}])]
