...
(defn- decode-transit* [^String s]
  (let [rdr (transit/reader (ByteArrayInputStream. (.getBytes s)) :json {:handlers serde/tj-read-handlers})]
    (loop [res []]
      (let [value-read (try
                         (transit/read rdr)
...
(defn- decode-json* [^String s]
  (let [json-lines (str/split-lines s)]
    (loop [res [] lns json-lines]
      (if-not (seq lns)
        res
        (recur (conj res (json/read-str (first lns))) (rest lns))))))
