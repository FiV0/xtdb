...
== Running Multi-Node Cluster Locally (Docker Compose)

For local testing of multi-node XTDB clusters, use the docker-compose setup in `docker-compose/`:

=== Build Images from Current Branch

[source,bash]
----
cd modules/bench/docker-compose
./build-images.sh
----

This builds both `xtdb/xtdb-aws:local` and `xtdb/xtdb-bench:local` from your current branch.
...
# Start infrastructure + 3 XTDB nodes
docker-compose up -d
...
=== Run AuctionMark Benchmark

[source,bash]
----
# Run with defaults (8 threads, 30 min, scale 0.1)
docker-compose --profile benchmark run --rm auctionmark

# Custom parameters
docker-compose --profile benchmark run --rm auctionmark \
  auctionmark -f /config/xtdb-config.yaml \
  --threads 8 --duration PT10M --scale-factor 0.1

# Two-phase: load then run
docker-compose --profile benchmark run --rm auctionmark \
  auctionmark -f /config/xtdb-config.yaml --only-load --scale-factor 0.1

docker-compose --profile benchmark run --rm auctionmark \
  auctionmark -f /config/xtdb-config.yaml --no-load --threads 8 --duration PT30M
----

=== Connect to Individual Nodes

* Node 1: `localhost:5432` (PGWire), `localhost:8081` (metrics)
* Node 2: `localhost:5433` (PGWire), `localhost:8082` (metrics)
* Node 3: `localhost:5434` (PGWire), `localhost:8083` (metrics)
...
To use GHCR images instead of locally built ones:

[source,bash]
----
XTDB_IMAGE=ghcr.io/xtdb/xtdb-aws:edge BENCH_IMAGE=ghcr.io/xtdb/xtdb-bench:nightly docker-compose up -d
----
